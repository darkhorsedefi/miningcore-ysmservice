...
public abstract class BitcoinJobManagerBase<TJob> : JobManagerBase<TJob>
{
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        else if(submitBlockResponse.Error?.Code == (int)BitcoinRPCErrorCode.RPC_MISC_ERROR || submitBlockResponse.Error?.Code == (int)BitcoinRPCErrorCode.RPC_INVALID_PARAMS || submitBlockResponse.Error?.Code == (int)BitcoinRPCErrorCode.RPC_INVALID_PARAMETER)
    ...
    protected virtual IDestination AddressToDestination(string address, BitcoinAddressType? addressType)
    {
        ...
        switch(addressType.Value)
        {
            ...
            case BitcoinAddressType.Decred:
                return BitcoinUtils.DecredAddressToDestination(poolConfig.Address, network);
            case BitcoinAddressType.Rincoin:
                return BitcoinUtils.RincoinAddressToDestination(poolConfig.Address, network);
            default:
